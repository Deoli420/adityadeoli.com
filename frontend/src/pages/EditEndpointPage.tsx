...
import type { ApiEndpointUpdate, AdvancedConfig } from "@/types/index.ts";
import type {
  KeyValuePair,
  AuthConfig,
  AuthType,
...
function hydrateKV(data: KeyValuePair[] | null | undefined): KeyValuePair[] {
  if (!data || data.length === 0) return [emptyKV()];
  return data.map((kv) => ({ ...kv, id: kv.id || uid() }));
}

/**
 * Edit Endpoint — loads an existing endpoint and allows editing all fields.
 *
 * Reuses the same V2 tabbed interface as CreateEndpointPage (Params, Headers,
 * Auth, Body, Cookies, Advanced) with test execution panel.
 */
export function EditEndpointPage() {
  ...
  // Core form fields
  const [form, setForm] = useState<ApiEndpointUpdate>({
    name: "",
    url: "",
    method: "GET",
    expected_status: 200,
  ...
  // Populate form when endpoint data loads
  useEffect(() => {
    if (endpoint && !initialized) {
      ...
      setParams(hydrateKV(endpoint.query_params));
      setHeaders(hydrateKV(endpoint.request_headers));
      setCookies(hydrateKV(endpoint.cookies));
      setMonitoringInterval(endpoint.monitoring_interval_seconds ?? 300);
      ...
      if (endpoint.body_config) {
        setBody({
          ...defaultBody(),
          ...endpoint.body_config,
          formData: endpoint.body_config.formData
            ? hydrateKV(endpoint.body_config.formData as unknown as KeyValuePair[])
            : [emptyKV()],
          urlEncoded: endpoint.body_config.urlEncoded
            ? hydrateKV(endpoint.body_config.urlEncoded as unknown as KeyValuePair[])
            : [emptyKV()],
        });
      }
      if (endpoint.advanced_config) {
        setAdvanced({ ...defaultAdvanced(), ...endpoint.advanced_config });
      }
      setInitialized(true);
    }
  }, [endpoint, initialized]);

  function updateField<K extends keyof ApiEndpointUpdate>(
    key: K,
    value: ApiEndpointUpdate[K],
  ) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const payload: ApiEndpointUpdate = {
      ...form,
      monitoring_interval_seconds: monitoringInterval,
      query_params: cleanKV(params) as ApiEndpointUpdate["query_params"],
      request_headers: cleanKV(headers) as ApiEndpointUpdate["request_headers"],
      cookies: cleanKV(cookies) as ApiEndpointUpdate["cookies"],
        ...
        <div className="flex items-center gap-3 mb-1">
          ...
          <div>
            ...
            <p className="text-[11px] text-text-tertiary">
              ...
              <span className="font-medium text-text-secondary">{endpoint.name}</span>
            </p>
          </div>
        </div>
            ...
            <div className="space-y-4 rounded-lg border border-border-subtle bg-surface-secondary/50 p-4">
              ...
              >
                <input
                  ...
                  onChange={(e) =>
                    setMonitoringInterval(parseInt(e.target.value, 10) || 300)
                  }
                  className="input font-mono text-[13px] tabular-nums"
                />
                <p className="mt-1 text-[10px] text-text-tertiary">
