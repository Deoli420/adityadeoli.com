...
function TimelineEvent({ event }: { event: IncidentEvent }) {
  ...
  return (
    <div className="relative flex gap-4 pb-6 last:pb-0">
      ...
      {/* Icon dot */}
      <div className="relative z-10 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-surface-tertiary ring-1 ring-border">
      ...
      <div className="min-w-0 flex-1 pt-1">
        ...
        <p className="mt-0.5 text-xs text-text-tertiary">{formatDate(event.created_at)}</p>
      </div>
    </div>
  );
}
  ...
  if (isLoading || !incident) {
    return (
      <div className="space-y-6">
        <div className="h-8 w-48 rounded bg-surface-tertiary animate-pulse" />
        <div className="card space-y-4 p-6">
          <div className="h-6 w-2/3 rounded bg-surface-tertiary animate-pulse" />
          <div className="h-4 w-1/2 rounded bg-border animate-pulse" />
          <div className="grid grid-cols-4 gap-4">
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="h-16 rounded-lg bg-surface-tertiary animate-pulse" />
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="card p-6 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          <div className="rounded-lg bg-surface-tertiary px-4 py-3">
            <p className="text-[11px] font-medium uppercase tracking-wider text-text-tertiary">Started</p>
            <p className="mt-1 text-sm font-medium text-text-primary">
              {formatDate(incident.started_at)}
            </p>
          </div>
          <div className="rounded-lg bg-surface-tertiary px-4 py-3">
            <p className="text-[11px] font-medium uppercase tracking-wider text-text-tertiary">Duration</p>
            <p className="mt-1 text-sm font-medium text-text-primary">
              {duration(incident.started_at, incident.resolved_at)}
            </p>
          </div>
          <div className="rounded-lg bg-surface-tertiary px-4 py-3">
            <p className="text-[11px] font-medium uppercase tracking-wider text-text-tertiary">Acknowledged</p>
            <p className="mt-1 text-sm font-medium text-text-primary">
              {incident.acknowledged_at ? formatDate(incident.acknowledged_at) : "—"}
            </p>
          </div>
          <div className="rounded-lg bg-surface-tertiary px-4 py-3">
            <p className="text-[11px] font-medium uppercase tracking-wider text-text-tertiary">Endpoint</p>
            <Link
        ...
        <div className="lg:col-span-3 space-y-4">
          ...
          {incident.notes && (
            <div className="card p-4">
              <div className="prose prose-sm max-w-none whitespace-pre-wrap text-sm text-text-secondary leading-relaxed">
                {incident.notes}
              </div>
            </div>
          )}
